...
# prefix is equivelent to writing /posts in the path operation
# e.g. @router.get("/posts", response_model= List[schemas.Post])
# tags will categorize the apis in the documentation /docs or /redoc
...
@router.post("/", status_code=status.HTTP_201_CREATED, response_model= schemas.Post) # default status code for a specific operation
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    # validate that only the owner can delete own post
...
@router.put("/{id}", response_model= schemas.Post)#, status_code=status.HTTP_205_RESET_CONTENT) # default status code for a specific operation
# reference class Post into variable post
